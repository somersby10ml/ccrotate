...
class CCRotate {
  ...
  async refresh() {
    ...
    console.log(chalk.blue('🔄 Testing accounts and refreshing tokens...\n'));

    // Start all tests with staggered delays
    emails.forEach((email, index) => {
      ...
      const testPromise = new Promise(async (resolve) => {
        ...
        process.stdout.write(chalk.gray(`Testing ${email}... `));
    ...
    console.log(chalk.bold(`${'Account'.padEnd(30)} ${'Status'.padEnd(10)} Response`));
    console.log(chalk.gray('─'.repeat(80)));

    results.forEach(result => {
      const statusColor = result.status === 'success' ? chalk.green : chalk.red;
      const statusText = result.status === 'success' ? '✓ Active' : '✗ Failed';
      
      console.log(
        `${result.email.padEnd(30)} ${statusColor(statusText.padEnd(10))} ${chalk.gray(result.response)}`
      );
    });
    ...
    console.log(chalk.blue(`\n🎯 Summary: ${successCount}/${results.length} accounts are active`));
  }
}
